...
### Added
- Added mixed remote server support (unix): the thin client can manage secondary local or SSH-backed Herdr servers from a client-rendered sidebar, persist the remote registry on the main server, show combined workspace and agent summaries with per-host banners (a state glyph plus the host name in the theme accent color), route workspace creation/focus/rename/close to the selected server, and keep secondary disconnects isolated from the main session.
...
- `herdr --remote <host>` sessions are no longer slowed to a crawl by supervisor polling: main-server registry/settings/summary state now fetches off the client UI loop with coalesced reruns, so subscribed change events and focus actions no longer block input and frame rendering for a WAN round-trip bundle each.
- Collapsing the composited multi-server sidebar now actually reclaims its width: the sidebar narrows to the compact glance rail (or fully hides with `ui.sidebar_collapsed_mode = "hidden"`) and every connected server re-renders at the wider content size, instead of drawing the collapsed layout inside the full expanded column.
- Mouse input over the content area reclaimed by a collapsed sidebar now reaches the terminal at the correct position instead of being swallowed or offset by the expanded width, and summary-refresh coalescing clears its queued state symmetrically on write failures and fleet fan-outs.
