...
When several servers are registered, the sidebar draws a banner row above each remote host's spaces. `[ui.sidebar.host]` styles those banners:
...
`gradient` colors the host name. The default, `solid`, uses one flat theme accent color with no shading or animation. The shaded presets are `rainbow`, `accent`, `cool`, `warm`, and `muted`; for those, `animation` (`animated` or `static`) and `speed` (`calm`, `normal`, or `lively`) control the gradient movement. `glyph` places the connection-state marker to the `left` of the name or hides it with `none`. `show_count` appends the host's space count to connected banners. `show_metrics` shows a live latency and download-rate readout at the banner's right edge; it is off by default. Unknown values fall back to the documented defaults instead of failing the config. All six options are also editable live from the settings "sidebar" section, which persists changes back to this table.
