...
When several servers are registered, the sidebar draws a banner row above each remote host's spaces: a connection-state glyph followed by the host name in the theme accent color, with a dim state suffix while a host is connecting, offline, disabled, or protocol-mismatched. The banner has no configuration options. A legacy `[ui.sidebar.host]` table from older builds is ignored.
